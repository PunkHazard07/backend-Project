...
exports.clearCart = async (req, res) => {
    try {
        const cart = await Cart.findOneAndUpdate({ user: req.user._id }, { items: [], total: 0 }, // Set total as a number
        { new: true });
        if (!cart) {
            return res
                .status(404)
...
exports.mergeCart = async (req, res) => {
    try {
        ...
        for (const localItem of items) {
            ...
            const existingItemIndex = cart.items.findIndex(item => item.productID && item.productID._id.toString() === productId.toString());
