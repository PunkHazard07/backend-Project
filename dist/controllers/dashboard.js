...
exports.getDashboardMetrics = async (req, res) => {
    try {
        const { timePeriod = 'all', ordersPage = 1, productsPage = 1, pageSize = 5 } = req.query;
        const skip = (page) => (page - 1) * pageSize;
        const { startDate, endDate } = getDateRange(timePeriod);
        // Execute all metrics queries in parallel for better performance
        const [totalOrders, totalSales, registeredUsers, outOfStockProducts, totalProducts, successfulPayments, failedPayments, recentOrders, totalOrdersCount, recentProducts, totalProductsCount] = await Promise.all([
            // Sales Metrics
